...
public:
  ...
  fg::FGraphPtr flowgraph_;//the control flow graph created by the flowgraph.cc.
  LiveGraph live_graph_;

  std::unique_ptr<graph::Table<assem::Instr, temp::TempList>> in_;//the table used to map the instr and its in liveness set.
  std::unique_ptr<graph::Table<assem::Instr, temp::TempList>> out_;//the table used to map the instr and its out liveness set.
  tab::Table<temp::Temp, INode> *temp_node_map_;//the table used to map a reg to its node in the interf_graph.
